		...
		for(jj=0; jj<n; jj+=blockSize){
			for(kk=0; kk<n; kk+=blockSize){
				for(i=0; i<blockSize && ii+i<n; ++i){
					for(j=0; j<blockSize && jj+j<n; j+=4){
						int const kStop = mymin(blockSize, n-kk);
						__m256d a_avx, b_avx, c_avx;
						c_avx = _mm256_load_pd(&c[(ii + i)*n + jj + j]);
						for(k=0; k<kStop; k+=4){
							a_avx = _mm256_load_pd(&a[(ii + i)*n + kk + k]);
							b_avx = _mm256_load_pd(&b[(kk + k)*n + jj + j]);
							c_avx = _mm256_fmadd_pd(a_avx, b_avx, c_avx);
						}
						_mm256_store_pd(&c[(ii + i)*n + jj + j], c_avx);
					}
				}
			}
		}
