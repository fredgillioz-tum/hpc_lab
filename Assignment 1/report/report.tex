...
The decline for higher problem sizes can be explained with cache size. %TODO: more detail

\subsection*{Cache-blocking}
Sample data for matrix-matrix-multiplication with cache blocking was recorded using the modified program \texttt{dgemm\_blocked.c} and the shell script \texttt{collect\_data2.sh}. Results for MFLOP rates can be seen in figure \ref{blocked_flops}. Results for cache-miss rate can be seen in figure \ref{blocked_cache}. %TODO: changes with higher n

\begin{figure}[h]
  \begin{center}
    \includegraphics{../graphics/graph_unblocked.pdf}	% TODO: replace with real graph
...
%note: cache-miss rate for avx wasn't demanded by the task
